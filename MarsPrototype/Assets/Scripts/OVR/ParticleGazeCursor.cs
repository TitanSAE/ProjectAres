...
public class ParticleGazeCursor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (particleTrail)
        {
			...
			ParticleSystem.EmissionModule em = psHalf.emission;
			ParticleSystem.MinMaxCurve rate = new ParticleSystem.MinMaxCurve();
			rate.constantMax = halfEmission.Evaluate((delta.magnitude / Time.deltaTime) / maxSpeed) * emissionScale;
			em.rate = rate; 

			ParticleSystem.EmissionModule em2 = psHalf.emission;
			ParticleSystem.MinMaxCurve rate2 = new ParticleSystem.MinMaxCurve();
			rate2.constantMax = fullEmission.Evaluate((delta.magnitude / Time.deltaTime) / maxSpeed) * emissionScale;
			em2.rate = rate2; 
